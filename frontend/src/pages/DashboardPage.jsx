...
const DashboardPage = () => {
  ...
  const user = useSelector((state) => state.auth.user); // Get user from the Redux store
  const [logout] = useLogoutMutation(); // Hook for logout mutation

  useEffect(() => {
    if (!user) {
      dispatch(useCheckAuthMutation());
    }
  }, [dispatch, user]);
  ...
  if (!user) {
    return <div>Loading...</div>; // Show a loading message if the user is not yet fetched
  }
